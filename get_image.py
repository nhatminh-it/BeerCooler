...
def resize_image(image, max_width=500, max_heigth=600):
    ...
    if w_factor > h_factor:
        image = image.resize((int(image.size[0] / w_factor), int(image.size[1] / w_factor)), Image.Resampling.LANCZOS)
    else:
        image = image.resize((int(image.size[0] / h_factor), int(image.size[1] / h_factor)), Image.Resampling.LANCZOS)
    return image
